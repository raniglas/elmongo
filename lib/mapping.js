				...
				if (pathInfo.instance) {
					...
					if (Array.isArray(typeClass)) {
						if (!typeClass[0]) {
							...
							currentLocation[pathEntry].type = exports.getElasticsearchTypeFromMongooseType(arrEntryTypeClass,typeClass[0])
						}
					} else {
						// `options` exists on pathInfo and it's not an array of types
						currentLocation[pathEntry].type = exports.getElasticsearchTypeFromMongooseType(typeClass,pathInfo.options)
					}
				}
...
exports.getElasticsearchTypeFromMongooseType = function (typeClass,info) {
	if (typeClass === String) {
		return 'string'
	}
	if (typeClass === Number) {
    ...
    if( typeof typeClass === "undefined") {
        if( info && 'lat' in info && 'lon' in info ) {
            return 'geo_point'
        }
    }
}